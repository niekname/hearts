...
class Game private constructor(events: List<Event> = emptyList()) {
    ...
    private fun PlayerWithCards.passCards() =
        CardsPassed.PlayerPassing(
            from = player,
            to = determineToPlayer(player),
            card1 = cards.elementAt(0),
            card2 = cards.elementAt(1),
            card3 = cards.elementAt(2)
        )

    private fun determineToPlayer(fromPlayer: Player) =
        when {
            isFirstHand() -> players().playerAtLeftSideOf(fromPlayer)
            isSecondHand() -> players().playerAtRightSideOf(fromPlayer)
            else -> players().playerAcross(fromPlayer)
        }
